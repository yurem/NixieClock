...
enum {DOT_POS, ANODE_4_POS, ANODE_3_POS, ANODE_2_POS, ANODE_1_POS, ANODE_5_POS, ANODE_6_POS}; //порядок анодов ламп(точки всегда должны быть первыми)(только для прямого подключения к микроконтроллеру)
const uint8_t digitMask[] = {9, 8, 0, 5, 4, 7, 3, 6, 2, 1, 10}; //тут вводим свой порядок пинов лампы(цифра "10" - это пустой символ, должен быть всегда в конце)
const uint8_t cathodeMask[] = {1, 0, 2, 9, 3, 8, 4, 7, 5, 6}; //порядок катодов in14
#endif
...
#define PCF8591_UPDATE_TIME 150        //таймаут обновления PCF8591 (мс)
