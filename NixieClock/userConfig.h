...
#define SENS_BME_ENABLE 2         //включить поддержку датчиков температуры BMPxxx и BMExxx(0 - датчик не используется(или используется на стороне esp) | 1 - используется датчик BMP085/BMP180 | 2 - используется датчик BMP280/BME280 | 3 - используются датчики BMP085/BMP180/BMP280/BME280)
...
#define DISABLE_SOME_BACKL_EFFECTS 1// отключает часть эффектов подсветки для уменьшения размера прошивки (освобождает 862 байт)
#define DISABLE_SOME_DOT_EFFECTS 1  // отключает часть эффектов точек для уменьшения размера прошивки (освобождает 148 байт)
#define DISABLE_GLITCH 1            // отключает глюки (освобождает 406 байт)
