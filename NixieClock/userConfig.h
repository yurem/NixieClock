...
#define BOARD_TYPE 4              //тип платы часов(0 - IN-12 (индикаторы стоят правильно) | 1 - IN-12 turned (индикаторы перевёрнуты) | 2 - IN-14 (обычная и неоновая точка) | 3 - другие индикаторы(4 лампы) | 4 - другие индикаторы(6 ламп))
...
#define AMP_WAIT_TIME 10         //время ожидания активации усилителя(10..150)(мс)
...
#define INDI_DOT_TYPE 0           //тип подключения 1-й неоновой лампы при использовании сдвигового регистра(0 - прямое подключение к микроконтроллеру | 1 - подключение через сдвиговый регистр)
...
#define GEN_FEEDBACK 1            //разрешить обратную связь вв преобразователя через делитель напряжения(0 - обратная связь не используется | 1 - обратная связь используется)
#define GEN_HV_VCC 170            //напряжение питания ламп(150..190)(в)
#define GEN_HV_R_HIGH 360         //сопротивление верхнего плеча резисторного делителя(кОм)
#define GEN_HV_R_LOW 4.7           //сопротивление нижнего плеча резисторного делителя(кОм)
...
#define LIGHT_SENS_PULL 1         //тип подтяжки сенсора яркости освещения(0 - сенсор подтянут к земле (LOW_PULL) | 1 - сенсор подтянут к питанию (HIGH_PULL))
...
#define BTN_RIGHT_R_HIGH 33       //сопротивление резистора "ПРАВОЙ" клавиши(полное сопротивление от кнопки до пина)(кОм)
...
#define DEFAULT_DOT_MODE 2         //режим работы точки по умолчанию(0 - выкл | 1 - статичная | 2 - динамичная(плавно мигает)), (для двух неоновых ламп)(3 - маятник), (для точек в индикаторах)(3 - бегущая | 4 - змейка | 5 - резинка | 6 - маятник(только для 6-ти ламп или если используются левые и правые точки))
...
#define DOT_TURN_TIME 500          //время переключения точек в индикаторах для анимации "маятник" (100..500)(0 - один раз в секунду)(мс)(только для разделительных или 2-х неоновых точек)
...
#define DEFAULT_AUTO_TEMP_TIME 1   //интервал автоматического показа температуры по умолчанию(0..15)(0 - выключить автоматический показ)(мин)
#define DEFAULT_AUTO_TEMP_ANIM 0   //режим анимации автоматического показа температуры по умолчанию(0 - режим зависит от основной анимации | 1 - случайная смена эффектов | 2 - плавное угасание и появление | 3 - перемотка по порядку числа | 4 - перемотка по порядку катодов в лампе | 5 - поезд | 6 - резинка | 7 - ворота | 8 - волна | 9 - блики | 10 - испарение | 11 - игровой автомат)
#define AUTO_TEMP_PHASE 30         //фаза отображения автоматического показа температуры(необходимо учитывать фазу отображения антиотравления и анимации смены минут)(0..59)(сек)
...
#define AUTO_TEMP_BACKL_TYPE 1     //режим подсветки во время автоматического показа температуры(0 - всегда выключена | 1 - только когда включена основная подсветка | 2 - всегда включена)(только для светодиодов WS2812B)
