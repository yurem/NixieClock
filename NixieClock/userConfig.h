...
#define DS3231_ENABLE 1           //включить поддержку модуля часов DS3231(0 - модуль не используется | 1 - используется только модуль часов | 2 - используется модуль часов и встроенный датчик температуры)
...
#define AMP_RDA_PORT_ENABLE 1     //перепрограммировать и включить использование порта управления питанием усилителя(0 - порт не используется | 1 - порт используется)
