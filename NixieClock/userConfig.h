...
#define DOTS_PORT_ENABLE 0        //включить использование порта разделительных точек в индикаторах(двух неоновых ламп)(0 - порт не используется | 1 - порт используется)
#define DOTS_TYPE 0               //тип разделительных точек в индикаторах(двух неоновых ламп)(0 - только левые точки | 1 - левые и правые точки)
...
#define PCF8591_ENABLE 0          //включить использование портов PCF8591 (0 - порт не используется | 1 - порт используется)
...
#define LIGHT_SENS_TYPE 1         //вид подключения сенсор яркости освещения(1 - сенсор подключён к Arduino | 2 - сенсор подключён к PCF8591)
...
#define BTN_ADD_PULL 1            //тип подтяжки цифровой дополнительной кнопки(0 - кнопка подтянута к земле (LOW_PULL) | 1 - кнопка подтянута к питанию (HIGH_PULL)) | 3 - цифровая кнопка подключенная ко второму входу PCF8591
...
#define DEFAULT_DOT_MODE 2         //режим работы точки по умолчанию(0 - выкл | 1 - статичная | 2 - динамичная(плавно мигает)), (для двух неоновых ламп)(3 - маятник), (для точек в индикаторах)(3 - бегущая | 4 - змейка | 5 - маятник(только для 6-ти ламп или если используются левые и правые точки))
...
#define DOT_TURN_TIME 500          //время переключения точек в индикаторах(или двух неоновых ламп) для анимации "маятник" (100..500)(0 - один раз в секунду)(мс)(только для разделительных или 2-х неоновых точек)
